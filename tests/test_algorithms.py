...
def test_bart_basic():
    ...
    if whynot_estimators.causal_bart.is_installed():
        _ = whynot_estimators.causal_bart.estimate_treatment_effect(covariates, treatment, outcome)


def test_causal_forest_basic():
    """Ensure causal forest algorithm runs."""
    covariates, treatment, outcome = generate_data(100, 10)
    _ = whynot_estimators.causal_forest.estimate_treatment_effect(covariates, treatment, outcome)
...
def test_deconfounder_basic():
    ...
    if whynot_estimators.deconfounder.is_installed():
        _ = whynot_estimators.deconfounder.estimate_treatment_effect(covariates, treatment, outcome)
...
def test_doubleml_basic():
    ...
    if whynot_estimators.doubleml.is_installed():
        _ = whynot_estimators.doubleml.estimate_treatment_effect(covariates, treatment, outcome)
...
def test_ipweighting_basic():
    ...
    if whynot_estimators.ip_weighting.is_installed():
        _ = whynot_estimators.ip_weighting.estimate_treatment_effect(covariates, treatment, outcome)
...
def test_matching_basic():
    ...
    if whynot_estimators.matching.is_installed():
        _ = whynot_estimators.matching.estimate_treatment_effect(covariates, treatment, outcome)
...
def test_rlearner_basic():
    ...
    if whynot_estimators.rlearner_mlp.is_installed():
        _ = whynot_estimators.rlearner_mlp.estimate_treatment_effect(covariates, treatment, outcome)
    if whynot_estimators.rlearner_xbg.is_installed():
        _ = whynot_estimators.rlearner_xbg.estimate_treatment_effect(covariates, treatment, outcome)
